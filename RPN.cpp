...
string Formula::ReplaceVariableWithDouble(double x, double y) {
    string convertedVariableX = to_string(x);
    string convertedVariableY = to_string(y);
    string formulaCopy = _formula;

    for (size_t i = 0; i < formulaCopy.length(); i++) {

        if (formulaCopy[i] == 'x') {
            formulaCopy.insert(i, convertedVariableX);

            if (formulaCopy[i] == '-') formulaCopy[i] = '~';
            i += convertedVariableX.length();

            formulaCopy.erase(i, 1);

        } else if (formulaCopy[i] == 'y') {
            formulaCopy.insert(i, convertedVariableY);

            if (formulaCopy[i] == '-') formulaCopy[i] = '~';
            i += convertedVariableY.length();
...
double Formula::Evaluate(double x, double y) {
    string stringModified = ReplaceVariableWithDouble(x, y);
